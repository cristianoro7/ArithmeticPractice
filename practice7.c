 ...
 * 接着求这两个指针的中间数.
 * 如果这个中间数大于第一个数的话, 说明中间数存在第一个有序数组中, 将第一个指针指向中间数. 如果中间数小于第二个数, 说明中间数位于第二个有序数组,
 * 将第二个指针指向中间数.
 *
 * 如此循环, 直到第一个指针指向第一个有序数组的末尾, 第二个指针指向第二个有序数组的开头. 此时找到最小了, 它是第二个指针指向的值.
 *
 * 特殊情况:
 * 1. 搬0个数到数组的末尾, 说明数组本身是有序的. 此时, 直接返回第一个数.
 * 2. 第一个指针指向的数, 第二个指针指向的数, 中间数 这三个数相等时, 我们只能顺序查找.
 */
